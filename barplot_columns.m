...
%                 handles.point_han{j, i} = plot(xvalues{i}(j), dat(j, i), mym, 'MarkerSize', mymarkersize, 'Color', mycolcolor ./ 2, 'LineWidth', 1, 'MarkerFaceColor', myc);
                if verLessThan('matlab','8.4')
                    ...
                    handles.point_han{j, i} = scatter(xvalues{i}(j), dat(j, i), mymarkersize, mycolcolor ./2 , mym, 'filled', 'MarkerFaceColor', myc, 'MarkerFaceAlpha', myalpha); %, 'LineWidth', 1, 'MarkerFaceColor', myc,'MarkerFaceAlpha',myalpha,'MarkerEdgeAlpha',myalpha);
                end
...
for i = 1:k
    ...
    for j = 1:nbins
        ...
        mylimit(j) = 0.2 * nanmean(myF(whu));  % average density for this 'slab' of points
